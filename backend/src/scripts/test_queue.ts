...
async function main() {
    ...
    // A fixed tenantId so this manual script exercises the tenant check
    // getJobStatus now enforces (REV-041) instead of tripping it.
    ...
    let status = await GenAIService.getJobStatus(job.id!, testTenantId);
    ...
    status = await GenAIService.getJobStatus(job.id!, testTenantId);
