...
export class WorkOrderService {
  ...
  static async create(data: CreateWorkOrderData) {
    ...
    const [newWorkOrder] = await db
      ...
      .values({
        ...
        estimatedHours: data.estimatedHours ? data.estimatedHours.toString() : undefined,
      })
      .returning();
  ...
  static async addTask(workOrderId: string, tenantId: string, data: { title: string; description?: string; taskOrder: number }) {
  ...
  static async updateTask(taskId: string, tenantId: string, data: { title?: string; description?: string; isCompleted?: boolean; notes?: string; completedBy?: string }) {
    ...
    const [updated] = await db.update(workOrderTasks)
      .set(updateData)
      .where(eq(workOrderTasks.id, taskId))
      .returning();
  ...
  static async addPart(workOrderId: string, tenantId: string, data: { partId: string; quantityRequired: number; notes?: string }) {
    const wo = await this.getById(workOrderId, tenantId);
  ...
  static async removePart(workOrderPartId: string, tenantId: string) {
    ...
    await db.delete(workOrderParts).where(eq(workOrderParts.id, workOrderPartId));
    return true;
  }
  ...
  static async addLabor(workOrderId: string, tenantId: string, data: { userId: string; hours: number; description?: string }) {
    const wo = await this.getById(workOrderId, tenantId);

    const [labor] = await db.insert(workOrderLabor).values({
      workOrderId: wo.id,
      userId: data.userId,
