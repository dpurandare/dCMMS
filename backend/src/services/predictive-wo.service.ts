...
export class PredictiveWOService {
    ...
    try {
      ...
      for (const prediction of highRiskPredictions) {
        try {
          ...
          console.error(
            `Failed to create WO for ${prediction.assetId}: ${errorMessage}`,
          );
        }
      }
      ...
      const errorMessage =
        error instanceof Error ? error.message : "Unknown error";
      console.error(`Predictive maintenance job failed: ${errorMessage}`);
      throw error;
    }
  }
  ...
  async createPredictiveWorkOrder(
    ...
    const topFactors = explanation.topFeatures
      .slice(0, 3)
      .map((f: { feature: string; shapValue: number }) => ({
        feature: f.feature,
    ...
    const factorsText = topFactors
      .map(
        (f: { feature: string; contribution: number }) =>
...
Recommendation: ${(explanation as { recommendation?: string }).recommendation || "Schedule inspection and preventive maintenance."}
...
Please review and approve/reject this work order in the dCMMS system.
   ...
   */
  private async getAssetOwner(_assetId: string): Promise<string> {
    // TODO: Query from database
   ...
   */
  async getStats(_startDate?: Date, _endDate?: Date) {
