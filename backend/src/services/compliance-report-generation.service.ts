...
export class ComplianceReportGenerationService {
  ...
  async generateReport(
    ...
    try {
      ...
      const [dbReport] = await db
        ...
        .values({
          tenantId,
          templateId: template.id, // This might need to be template.id (UUID) if schema expects UUID. Schema says templateId references complianceReportTemplates.id (UUID).
          // template.templateId is likely the string ID (e.g. "ISO-9001"). template.id is the UUID.
          // I need to check mapTemplate in ComplianceTemplateService.
          // mapTemplate returns templateId as dbTemplate.templateId.
          // I should probably fetch the UUID.
          // Wait, schema says templateId references complianceReportTemplates.id.
          // So I need the UUID of the template.
          // ComplianceTemplateService.getTemplate returns ComplianceTemplate interface which has id and templateId.
          // I should use template.id.
  ...
  private async generatePDF(
    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        // Header
        this.addPDFHeader(doc, template, reportName, watermark);
  ...
  private mapReport(dbReport: any): GeneratedReport {
    return {
      id: dbReport.id,
      reportId: dbReport.reportName, // Map reportName to reportId for interface compatibility
