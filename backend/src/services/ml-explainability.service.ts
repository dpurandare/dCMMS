...
export class MLExplainabilityService {
  ...
  async explainPrediction(params: { modelName: string; assetId: string }): Promise<any> {
  ...
  async getFeatureImportance(modelName: string, startDate?: Date, endDate?: Date): Promise<any> {
    return [
