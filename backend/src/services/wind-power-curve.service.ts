...
export class WindPowerCurveService {
  ...
  calculateExpectedPower(
    ...
    if (
      windSpeed < powerCurve.cut_in_speed ||
      windSpeed > powerCurve.cut_out_speed
    ) {
      return 0;
    }

    // 2. Sort points by wind speed to ensure correct interpolation
    const sortedPoints = [...powerCurve.points].sort(
      (a, b) => a.wind_speed - b.wind_speed,
    );
    ...
    for (let i = 0; i < sortedPoints.length - 1; i++) {
      ...
      if (windSpeed >= p1.wind_speed && windSpeed <= p2.wind_speed) {
        ...
        const slope =
          (p2.power_output - p1.power_output) / (p2.wind_speed - p1.wind_speed);
        const interpolatedPower =
          p1.power_output + (windSpeed - p1.wind_speed) * slope;
    ...
    if (windSpeed >= sortedPoints[sortedPoints.length - 1].wind_speed) {
      return Math.min(
        sortedPoints[sortedPoints.length - 1].power_output,
        powerCurve.rated_power,
      );
    }
