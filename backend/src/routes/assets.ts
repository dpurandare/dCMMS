...
const assetRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get(
    ...
    async (request, reply) => {
      ...
      const result = await AssetService.list(user.tenantId, filters, pagination);
      return result;
    }
  );
  ...
  server.post(
      ...
      schema: {
        ...
        body: {
          ...
          properties: {
            ...
            status: { type: 'string', enum: ['operational', 'down', 'maintenance', 'retired'] },
    ...
    async (request, reply) => {
      ...
      const assetTag =
        body.assetTag || (await AssetService.generateAssetTag(user.tenantId, body.siteId, body.type));
  ...
  server.patch(
    ...
    async (request, reply) => {
      ...
      if (body.lastMaintenanceDate) updates.lastMaintenanceDate = new Date(body.lastMaintenanceDate);
