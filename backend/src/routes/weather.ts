...
const weatherRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get(
    "/stored/:siteId",
    {
      schema: {
        description:
          "Get stored weather forecasts for a site within a date range",
        tags: ["weather"],
        security: [{ bearerAuth: [] }],
        params: {
          type: "object",
  ...
  server.get(
    ...
    async (request, reply) => {
      ...
      if (!forecast) {
        return reply
          .code(404)
          .send({ error: "No weather forecast found for this site" });
      }
  ...
  server.post(
    ...
    async (request, reply) => {
      ...
      const currentWeather =
        await weatherAPIService.fetchCurrentWeather(siteLocation);
      const forecasts = await weatherAPIService.fetchForecast(siteLocation);
