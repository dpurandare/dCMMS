...
const modelPerformanceRoutes: FastifyPluginAsync = async (server) => {
  ...
  server.get(
    ...
    async (request, reply) => {
      try {
        ...
        const metrics = await performanceService.calculateMetrics(modelName, windowSize);
  ...
  server.post(
    ...
    async (request, reply) => {
      try {
        const { modelName, versionA, versionB } = request.body as { modelName: string; versionA: string; versionB: string };
  ...
  server.post(
    ...
    async (request, reply) => {
      try {
        ...
        await performanceService.acknowledgeAlert(safeAlertId, (request.user as any)?.id || 'system');
  ...
  server.get(
    ...
    async (request, reply) => {
      try {
        ...
        const accuracy = await performanceService.getPredictionAccuracyByRisk(modelName);
