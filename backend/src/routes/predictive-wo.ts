...
const predictiveWORoutes: FastifyPluginAsync = async (server) => {
  ...
  server.post(
    '/run',
    {
      schema: {
        ...
        response: {
          ...
          403: { description: 'Forbidden (admin only)', type: 'object', properties: { error: { type: 'string' } } },
    ...
    async (request, reply) => {
      try {
        const { modelName = 'predictive_maintenance' } = request.body as { modelName?: string };
        ...
        response: {
          200: {
            ...
            properties: {
              ...
              avgTimeToApproval: { type: 'number', description: 'Average time in hours' },
